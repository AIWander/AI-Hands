...
pub async fn handle(
    ...
    let url = match args.get("url").and_then(|v| v.as_str()) {
        ...
        None => {
            instrumentation::log_aggregate(
                "hands_navigate", &call_id, false, "", 0, 0, None, Some("url is required"),
            );
            return MetaToolResult::failure(
                vec![], MetaError::other("url is required"), 0,
            ).to_value();
        }
    };

    let wait_condition = args.get("wait_condition").and_then(|v| v.as_str()).map(|s| s.to_string());
    ...
    if !browser_active {
        if visible {
            ...
            let dl_result = browser_mcp::tools::handle_tool(
                browser, "debug_launch", json!({"port": 9222}),
            ).await;
            ...
            if dl_ok {
                ...
                instrumentation::log_rung_attempt("hands_navigate", &call_id, "debug_launch", true, rung_ms, None, &ctx);

                let attach_start = Instant::now();
                let attach_result = browser_mcp::tools::handle_tool(
                    browser, "attach", json!({"port": 9222}),
                ).await;
                let (attach_ok, attach_val) = super::browser_result_to_value(attach_result);
                let attach_ms = attach_start.elapsed().as_millis() as u64;

                if !attach_ok {
                    let err = attach_val.get("error").and_then(|v| v.as_str()).unwrap_or("attach failed");
                    rungs_tried.push(RungAttempt::failed("attach", attach_ms, err));
                    instrumentation::log_rung_attempt("hands_navigate", &call_id, "attach", false, attach_ms, None, &ctx);

                    let elapsed = start.elapsed().as_millis() as u64;
                    instrumentation::log_aggregate_with_context(
                        "hands_navigate", &call_id, false, "", rungs_tried.len(), elapsed, None,
                        Some(&json!({"category": "subsystem_unavailable", "detail": err})), Some(&ctx),
                    );
                    return MetaToolResult::failure(
                        rungs_tried, MetaError::subsystem("browser_attach", err), elapsed,
                    ).with_reversibility(Reversibility::Reversible).to_value();
                }

                rungs_tried.push(RungAttempt::ok("attach", attach_ms));
                instrumentation::log_rung_attempt("hands_navigate", &call_id, "attach", true, attach_ms, None, &ctx);
            } else {
                rungs_tried.push(RungAttempt::failed("debug_launch", rung_ms, "Debug launch failed"));
                instrumentation::log_rung_attempt("hands_navigate", &call_id, "debug_launch", false, rung_ms, None, &ctx);

                // Fallback: headful launch
                let fb_start = Instant::now();
                let fb_result = browser_mcp::tools::handle_tool(
                    browser, "launch", json!({"headless": false}),
                ).await;
                let (fb_ok, fb_val) = super::browser_result_to_value(fb_result);
                let fb_ms = fb_start.elapsed().as_millis() as u64;

                if !fb_ok {
                    let err = fb_val.get("error").and_then(|v| v.as_str()).unwrap_or("launch failed");
                    rungs_tried.push(RungAttempt::failed("launch_headful", fb_ms, err));
                    instrumentation::log_rung_attempt("hands_navigate", &call_id, "launch_headful", false, fb_ms, None, &ctx);

                    let elapsed = start.elapsed().as_millis() as u64;
                    instrumentation::log_aggregate_with_context(
                        "hands_navigate", &call_id, false, "", rungs_tried.len(), elapsed, None,
                        Some(&json!({"category": "browser_not_running", "detail": err})), Some(&ctx),
                    );
                    return MetaToolResult::failure(
                        rungs_tried, MetaError::no_browser(), elapsed,
                    ).to_value();
                }

                rungs_tried.push(RungAttempt::ok("launch_headful", fb_ms));
                instrumentation::log_rung_attempt("hands_navigate", &call_id, "launch_headful", true, fb_ms, None, &ctx);
            }
        } else {
            ...
            if !ok {
                ...
                instrumentation::log_rung_attempt("hands_navigate", &call_id, "launch_headless", false, rung_ms, None, &ctx);
            ...
            instrumentation::log_rung_attempt("hands_navigate", &call_id, "launch_headless", true, rung_ms, None, &ctx);
        }
    ...
    let is_lifecycle = wait_condition.as_deref()
    ...
    if !nav_ok {
        let err = nav_val.get("error").and_then(|v| v.as_str()).unwrap_or("navigate failed");
        rungs_tried.push(RungAttempt::failed("navigate", nav_ms, err));
        instrumentation::log_rung_attempt("hands_navigate", &call_id, "navigate", false, nav_ms, None, &ctx);

        let elapsed = start.elapsed().as_millis() as u64;
        instrumentation::log_aggregate_with_context(
            "hands_navigate", &call_id, false, "", rungs_tried.len(), elapsed, None,
            Some(&json!({"category": "navigate_failed", "detail": err})), Some(&ctx),
        );
        return MetaToolResult::failure(
            rungs_tried, MetaError::other(format!("Navigate to '{}' failed", url)), elapsed,
        ).with_reversibility(Reversibility::Reversible).to_value();
    }

    rungs_tried.push(RungAttempt::ok("navigate", nav_ms));
    instrumentation::log_rung_attempt("hands_navigate", &call_id, "navigate", true, nav_ms, None, &ctx);
    ...
    let _ = browser_mcp::tools::handle_tool(
        browser, "wait_idle", json!({"timeout_ms": 1000}),
    ).await;

    // Step 4: Wait for CSS selector (non-lifecycle condition)
    let is_css_wait = wait_condition.as_deref()
        .map(|c| !LIFECYCLE_CONDITIONS.contains(&c))
        .unwrap_or(false);
    ...
    if is_css_wait {
        ...
        if !wait_ok {
            ...
            instrumentation::log_rung_attempt("hands_navigate", &call_id, "wait_for_selector", false, wait_ms, None, &ctx);

            let current_url = get_current_url(browser).await;
            let elapsed = start.elapsed().as_millis() as u64;

            let mut result = MetaToolResult::success(
                "navigate_partial", rungs_tried.clone(),
                json!({
                    "url": url,
            ...
            result = result.with_warning(
                format!("Navigated to '{}' but wait_for '{}' timed out", url, cond)
            );

            instrumentation::log_aggregate(
                "hands_navigate", &call_id, true, "navigate_partial",
                rungs_tried.len(), elapsed, None, None,
            );
            return result.to_value();
        }

        rungs_tried.push(RungAttempt::ok("wait_for_selector", wait_ms));
        instrumentation::log_rung_attempt("hands_navigate", &call_id, "wait_for_selector", true, wait_ms, None, &ctx);
    }
    ...
    instrumentation::log_aggregate(
        "hands_navigate", &call_id, true, "navigate",
        rungs_tried.len(), elapsed, Some(1.0), None,
    );
