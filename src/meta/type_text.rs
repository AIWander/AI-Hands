...
async fn type_via_desktop(
    ...
    if clear_first {
        let _ = uia_lib::handle_tool_call(
            "uia_key_press", &json!({"keys": "ctrl+a"}),
        );
        let _ = uia_lib::handle_tool_call(
            "uia_key_press", &json!({"keys": "delete"}),
        );
    }

    // Type via UIA
    let type_result = uia_lib::handle_tool_call(
        "uia_type", &json!({"text": text}),
    );
...
async fn type_via_window_fallback(
    ...
    let result = uia_lib::handle_tool_call(
        "uia_type", &json!({"text": text, "window_title": target}),
    );
