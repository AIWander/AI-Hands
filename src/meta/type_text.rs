...
async fn type_via_desktop(
    ...
    // Click to focus
    let click_result = uia_lib::handle_tool_call("uia_click", &json!({"x": x, "y": y}));
    ...
    if clear_first {
        let _ = uia_lib::handle_tool_call("uia_key_press", &json!({"keys": "ctrl+a"}));
        let _ = uia_lib::handle_tool_call("uia_key_press", &json!({"keys": "delete"}));
    }

    // Type via UIA
    let type_result = uia_lib::handle_tool_call("uia_type", &json!({"text": text}));
...
async fn type_via_window_fallback(
    ...
    let result =
        uia_lib::handle_tool_call("uia_type", &json!({"text": text, "window_title": target}));
    let ok = result
        .get("success")
        .and_then(|v| v.as_bool())
        .unwrap_or(false);
