...
async fn handle_close(
    ...
    if let Some(ref hwnd) = target.hwnd {
        let _ = uia_lib::handle_tool_call(
            "uia_focus_window",
            &json!({"hwnd": hwnd}),
        );
    } else {
    ...
    while dialog_poll_start.elapsed() < dialog_poll_deadline {
        ...
        let uia_dialog = uia_lib::handle_tool_call(
            "uia_find",
            &json!({"role": "dialog", "max_results": 1}),
        );
...
fn verify_foreground_window(
    ...
    let fg_result = uia_lib::handle_tool_call("uia_get_state", &json!({"property": "foreground_window"}));
