...
fn launch_application(spec: &str) -> Value {
    ...
    let _ = uia_lib::handle_tool_call("uia_key_press", &json!({"keys": "win"}));
    std::thread::sleep(Duration::from_millis(300));
    let _ = uia_lib::handle_tool_call("uia_type_text", &json!({"text": spec}));
    std::thread::sleep(Duration::from_millis(150));
    let _ = uia_lib::handle_tool_call("uia_key_press", &json!({"keys": "enter"}));
...
async fn handle_close(
    ...
    if let Some(ref hwnd) = target.hwnd {
        let _ = uia_lib::handle_tool_call("uia_focus_window", &json!({"hwnd": hwnd}));
    } else {
        let _ = uia_lib::handle_tool_call("uia_focus_window", &json!({"title": &target_title}));
    }
    let rung_ms = rung_start.elapsed().as_millis() as u64;
    ...
    // Send Alt+F4
    let rung_start = Instant::now();
    let _ = uia_lib::handle_tool_call("uia_key_press", &json!({"keys": "alt+f4"}));
    let rung_ms = rung_start.elapsed().as_millis() as u64;
    ...
    while dialog_poll_start.elapsed() < dialog_poll_deadline {
        ...
        let uia_dialog =
            uia_lib::handle_tool_call("uia_find", &json!({"role": "dialog", "max_results": 1}));
        ...
        for btn_name in &probe_buttons {
            let probe = uia_lib::handle_tool_call(
                "uia_find",
                &json!({"name": btn_name, "role": "button", "max_results": 1}),
            );
        ...
        match save_dialog::resolve_dialog_action(&dialog_info, on_save_dialog, app_name) {
            Ok(resolution) => {
                ...
                if let Some(ref button) = resolution.button_text {
                    let rung_start = Instant::now();
                    let _ = uia_lib::handle_tool_call("uia_click", &json!({"name": button}));
                    let rung_ms = rung_start.elapsed().as_millis() as u64;
            ...
            Err(e) => {
                if force_close {
                    ...
                    if let Some(ref proc) = target.process_name {
                        let _ =
                            uia_lib::handle_tool_call("uia_key_press", &json!({"keys": "escape"}));
                        tokio::time::sleep(Duration::from_millis(200)).await;
                        // Try Alt+F4 again, or the caller can use taskkill
                        let _ =
                            uia_lib::handle_tool_call("uia_key_press", &json!({"keys": "alt+f4"}));
...
async fn handle_focus(
    ...
    let (focus_result, used_hwnd) = if let Some(ref hwnd) = target.hwnd {
        ...
        if ok {
            ...
            let r2 =
                uia_lib::handle_tool_call("uia_focus_window", &json!({"title": &target_title}));
            (r2, false)
        }
    } else {
...
async fn handle_window_state(
    ...
    let state_result = uia_lib::handle_tool_call("uia_window_state", &state_args);
    let rung_ms = rung_start.elapsed().as_millis() as u64;
...
async fn handle_snap(
    ...
    let snap_result = uia_lib::handle_tool_call("uia_window_snap", &snap_args);
    let rung_ms = rung_start.elapsed().as_millis() as u64;
...
/// List all windows via uia_list_window, returning the array of window objects.
fn list_windows() -> Vec<Value> {
    let result = uia_lib::handle_tool_call("uia_list_window", &json!({}));
    result
        .get("windows")
...
fn verify_foreground_window(
    ...
    let fg_result =
        uia_lib::handle_tool_call("uia_get_state", &json!({"property": "foreground_window"}));
