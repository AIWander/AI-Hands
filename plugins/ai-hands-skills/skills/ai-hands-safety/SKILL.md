...
Tool booleans such as `allow_destructive=true` are not user consent. This skill is behavioral guidance and cannot block a tool call; obtain action-scoped confirmation through the host's native path. Native host permissions and the Rust monitor fence remain authoritative.

```text
Action check: RequiresConfirmation: <exact effect and target>; origin: <domain or application>
```

Use a separate Windows session or virtual machine when display isolation is security-critical.
