...
- Load `ai-hands-safety` before a consequential mutation.
- Ask before destructive, financial, external-send, account, or permission-changing actions.
- Tool booleans are not user consent. This skill cannot block a call, so obtain action-scoped confirmation through the host's native path.
...
Skills are behavioral guidance, not hard enforcement. The Rust Hands process owns strict monitor enforcement. Native host permissions remain authoritative.
